...
EXCLUDED_DIRS = {
    ...
    # .NET-specific
    ".vs", "packages", "TestResults", "artifacts",
}
...
def acquire_repo(source: str, scan_id: str) -> tuple[str, str, bool]:
    ...
    try:
        Repo.clone_from(
            ...
            multi_options=["--depth=1", "-c", "core.longpaths=true"],
        )
    except Exception as e:
        err_str = str(e)
        if "Filename too long" in err_str or "filename too long" in err_str:
            raise RuntimeError(
                "Clone failed because the repository contains file paths that exceed "
                "Windows' path-length limit. To fix this permanently, run:\n"
